...
class Person :
    ...
    def getEndPointScore(self, link) :
        #int(link.endpoint.name, link.endpoint.scores)
...
class Group(Person):
    ...
    def finish_travel(self) :
        if self.travelling :
            ...
            if self.places_travelled == 1 and SimulationSettings.StartOnFoot :  # First journey
                self.distance_travelled_on_link += self.speed_adjust(SimulationSettings.MaxWalkSpeed)
                self.distance_moved_this_timestep += self.speed_adjust(SimulationSettings.MaxWalkSpeed)
            else :
                self.distance_travelled_on_link += self.speed_adjust(SimulationSettings.MaxMoveSpeed)
                self.distance_moved_this_timestep += self.speed_adjust(SimulationSettings.MaxMoveSpeed)
...
#        self.agents.append(Person(location))
    ...
    def evolve(self) :
            ...
            for loc in sample_locs:
                    ...
                    # remove old group from the location and add two new ones
