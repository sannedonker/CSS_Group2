...
class Person:
  ...
  __slots__ = ['location', 'home_location', 'timesteps_since_departure', 'places_travelled', 'recent_travel_distance',
               'distance_moved_this_timestep', 'travelling', 'distance_travelled_on_link', 'move_speed', 'walk_speed']

  def __init__(self, location, speed):
    self.move_speed = speed * SimulationSettings.MaxMoveSpeed
    self.walk_speed = speed * SimulationSettings.MaxWalkSpeed
  ...
  def finish_travel(self):
    if self.travelling:
      ...
      if self.places_travelled == 1 and SimulationSettings.StartOnFoot: # First journey
        self.distance_travelled_on_link += self.walk_speed
        self.distance_moved_this_timestep += self.walk_speed
      else:
        self.distance_travelled_on_link += self.move_speed
        self.distance_moved_this_timestep += self.move_speed
      ...
      if self.distance_travelled_on_link > self.location.distance:
        ...
        if self.location.closed == True:
          ...
          evolveMore = False
          if self.distance_moved_this_timestep < self.move_speed:
            evolveMore = True
          ...
          if evolveMore == True:
            ...
            if SimulationSettings.AvoidShortStints:
              if (self.recent_travel_distance + ( self.distance_moved_this_timestep / self.move_speed )) / 2.0 < 0.5: # Flee 2.0 Changeset 1, factor 2.
                ForceTownMove = True
...
class Group(Person):
    ...
    def __init__(self, location, size, speed) :
        self.move_speed = speed * SimulationSettings.MaxMoveSpeed
        self.walk_speed = speed * SimulationSettings.MaxWalkSpeed
        self.location = location
        self.size = size  # Group size
        self.home_location = location
        [self.location.IncrementNumAgents() for i in range(self.size)]  ## Times group size
        self.timesteps_since_departure = 0
        self.places_travelled = 1
    ...
    def choose_link_form_list(self, n) :
        s = list(set(n))

        c = [n.count(i) for i in s]

        return s[c.index(max(c))]
    ...
    def evolve(self, ForceTownMove=False) :
        ...
        if self.travelling == False :
            ...
            if outcome < movechance :
                # determine here which route to take?
                if SimulationSettings.UseV1Rules :
                    chosenRoute_func = self.selectRouteRuleset1
                else :
                    chosenRoute_func = self.selectRouteRuleset2

                options = []
                for p in range(self.size) :
                    options.append(chosenRoute_func())
                if options==[]:
                    chosenRoute = -1
                else:
                    chosenRoute = self.choose_link_form_list(options)
                

                # if there is a viable route to a different location.
                if chosenRoute >= 0 :
                    # update location to link endpoint
                    [self.location.DecrementNumAgents() for i in range(self.size)]  ## Times group size
                    self.location = self.location.links[chosenRoute]
                    [self.location.IncrementNumAgents() for i in range(self.size)]
                    self.travelling = True
                    self.distance_travelled_on_link = 0
            else:
                return -1
    ...
    def speed_adjust(self, speed) :
        Y = 0.9
        return (Y) * speed + (1 - Y) * speed / self.size
    ...
    def finish_travel(self) :
        if self.travelling :
            ...
            if self.distance_travelled_on_link > self.location.distance :
                ...
                # if link is closed, bring agent to start point instead of the destination and return.
                if self.location.closed == True :
                    [self.location.DecrementNumAgents() for i in range(self.size)]
                    self.location = self.location.startpoint
                    [self.location.IncrementNumAgents() for i in range(self.size)]
                    self.travelling = False
                    self.distance_travelled_on_link = 0
                    ...
                    if evolveMore == True :
                        ...
                        if SimulationSettings.AvoidShortStints :
                            if (self.recent_travel_distance + (
                                    self.distance_moved_this_timestep / self.move_speed )) / 2.0 < 0.5 :  # Flee 2.0 Changeset 1, factor 2.
                                ForceTownMove = True
                        self.evolve(ForceTownMove)
                        self.finish_travel()
...
class Location :
    ...
    def print(self) :
        ...
        self.name, self.x, self.y, self.movechance, self.capacity, self.pop, self.country, self.conflict, self.camp),
        file=sys.stderr)
        for l in self.links :
            print("Link from %s to %s, dist: %s, pop. %s" % (self.name, l.endpoint.name, l.distance, l.numAgents), file
                  =sys.stderr)
...
class Ecosystem :
        ...
        if len(self.closures) > 0 :
            for c in self.closures :
                if time == c[3] :
                    if c[0] == "country" :
                        if Debug :
                            print("Time = %s. Closing Border between [%s] and [%s]" % (time, c[1], c[2]), file
                                  =sys.stderr)
                ...
                if time == c[4] :
                    if c[0] == "country" :
                        if Debug :
                            print("Time = %s. Reopening Border between [%s] and [%s]" % (time, c[1], c[2]), file
                                  =sys.stderr)
    ...
    def _remove_link_1way(self, startpoint, endpoint, close_only=False) :
        ...
        if not removed :
            print("Warning: cannot remove link from %s, as there is no link to %s" % (startpoint, endpoint), file
                  =sys.stderr)
        return removed
    ...
    def _reopen_link_1way(self, startpoint, endpoint) :
        ...
        if not reopened :
            print("Warning: cannot reopen link from %s, as there is no link to %s" % (startpoint, endpoint), file
                  =sys.stderr)
        return reopened
    ...
    def _change_location_1way(self, location_name, mode="close", direction="both", Debug=False) :
        ...
        print("%s location 1 way [%s] in direction %s (%s)." % (mode, location_name, direction, dir_mode), file
              =sys.stderr)
    ...
    def _change_border_1way(self, source_country, dest_country, mode="close", Debug=False) :
                ...
                while j < len(link_set) :
                    if link_set[j].endpoint.country == dest_country :
                        if Debug :
                            ...
                            mode, source_country, self.locations[i].name, dest_country, link_set[j].endpoint.name), file
                                  =sys.stderr)
                        changed_anything = True
                        if mode == "close" :
                            if self.close_link(self.locationNames[i], link_set[j].endpoint.name, twoway=False) :
        ...
        if not changed_anything :
            print(
            "Warning: no link closed when closing borders between %s and %s." % (source_country, dest_country), file
            =sys.stderr)
...
##############################################################################################################################################
    ...
    def addAgent_rout(self, location, size) :
        if SimulationSettings.TakeRefugeesFromPopulation :
            if location.conflict :
                if location.pop - size > 0 :
                    location.pop -= size
                    location.numAgentsSpawned += size
        ...
        self.agents.append(Group(location, size))
                

    def insertAgent(self, location,size) :
        """
...
#        self.agents.append(Person(location))
    ...
    def insertAgents(self, location, size, number) :
    ...
    def addAgent(self, location, size) :
        if SimulationSettings.TakeRefugeesFromPopulation :
            if location.conflict :
                if location.pop - size > 0 :
                    location.pop -= size
...
#                    location.print()
                    assert location.pop > 1
        #speed = np.random.normal(1, 0.1)
        speed = np.random.choice(np.arange(0,30,0.1))
        self.agents.append(Group(location, size, speed))
        
    def addAgent_speed(self, location, size,speed) :
        if SimulationSettings.TakeRefugeesFromPopulation :
            if location.conflict :
                if location.pop - size > 0 :
                    location.pop -= size
...
#                    location.print()
    ...
    def add_agents_to_conflict_zones(self, new_refs) :   
        """
        Form family groups from new refuges in current timestep and add them distributed across conflict zones.
        """
        mu, sigma = 2., 0.5
        groups_n = 0
        group_sizes = []
        ...
        cl = self.pick_conflict_locations(groups_n)        
        for i in range(0, groups_n) :
            self.addAgent(cl[i], group_sizes[i])
            
        '''
        Form larger groups from all agents at 40% of conlit zones at each time steps 
        '''
        
        sample_cl=random.sample(list(cl),int(len(cl)*0.4))
        #print("len temp_cl",len(temp_cl))
        
        for clcl in sample_cl:
