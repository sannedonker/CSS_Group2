...
class Person:
  ...
  def __init__(self, location, speed):
    self.move_speed = speed * SimulationSettings.MaxMoveSpeed   # Assign individual speed to agents 
...
class Ecosystem:
  ...
  def evolve(self):
    ...
    for a in self.agents:
      ...
      a.recent_travel_distance = (a.recent_travel_distance + ( a.distance_moved_this_timestep / a.move_speed )) / 2.0
  ...
  def addAgent(self, location):
    ...
    # Multiply the original speed by some random number
    
    #speed = np.abs(np.random.normal(1,0.1))
    speed = np.random.choice(np.arange(0,10,0.1))

    self.agents.append(Person(location, speed))
