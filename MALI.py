...
def AddInitialRefugees(e, d, loc):
  ...
  for i in range(0, num_refugees):
    e.addAgent(loc,1)  #uncomment in case fleeGroup and fleeGroupSpeed
    #e.addAgent(loc)  # uncomment in case fleeOriginal or fleeOriginalSpeed
...
# Niger
  ...
  reals=[Mbera_real,Mentao_real,Bobo_Dioulasso_real,Abala_real,Mangaize_real,Niamey_real,Tabareybarey_real]
  
  simuls=[Mbera_simu,Mentao_simu,Bobo_Dioulasso_simu,Abala_simu,Mangaize_simu,Niamey_simu,Tabareybarey_simu]
...
#########################################################################################
  ...
  for t in range(0,end_time):
    ...
    for i in range(0, len(camp_names)):
      ...
      simuls[i].append(camps[i].numAgents)
      reals[i].append(camp_pops[-1])
...
#for i in range(0, len(camp_names)):
#    np.savetxt("Mali_%s_real_original_too_speed.txt"%camp_names[i], reals[i] )
#    
#for i in range(0, len(camp_names)):
#    np.savetxt("Mali_%s_simuls_original_too_speed.txt"%camp_names[i], simuls[i] )
#
#
#
#np.savetxt("Mali_Error_original_speed",errors)
#plt.plot(errors)
#plt.title("Mali, Error, original_speed")
#plt.savefig("Mali_Error_original_speed")
#
#plt.show()
...
for i in range(0, len(camp_names)): 
    ...
    plt.plot(reals[i],label="Real Data",linewidth=3,color="red")
    plt.plot(simuls[i],label="Simulated model")

    plt.title("%s"%camp_names[i])
    if k==1 or k==4:
        plt.ylabel("Numer of refugees", fontsize=12)
